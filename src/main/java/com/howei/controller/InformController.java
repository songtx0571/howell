...
public class InformController {
    ...
    public String getCompanyList(HttpServletRequest request) {
        ...
        Subject subject = SecurityUtils.getSubject();
        Users user = (Users) subject.getPrincipal();
        boolean bool = subject.isPermitted("选择公司");
        ...
        if (bool) {
            ...
            Company company = companyService.getCompanyById(companyId.toString());
            list.add(company);
        }
    ...
    public String getUsersList(HttpServletRequest request) {
        ...
        Users user = (Users) subject.getPrincipal();
        boolean bool = subject.isPermitted("选择公司");
        boolean bool1 = subject.isPermitted("选择部门");
        ...
        maplist = userService.getUsersList(map);
    ...
    public String getDepartmentList(HttpServletRequest request) {
        ...
        Subject subject = SecurityUtils.getSubject();
        Users user = (Users) subject.getPrincipal();
        boolean bool = subject.isPermitted("选择部门");
        if (bool) {
            String companyId = request.getParameter("parent");
            list = companyService.getDepartmentList(companyId);
        } else {
    ...
    @GetMapping("/del")
    @ResponseBody
    public Result deleteInform(String informId) {
        Result result = new Result();
        int conunt = informService.deleteById(informId);
        if (conunt > 0) {
            result.setMsg("删除成功");
        } else {
